...
1. **Vector de prueba SHA-256.** `hash --format sha256 --string "abc"` produce
   `ba7816bf...015ad`, coincidente con FIPS 180-4 Apéndice B.1. → la
   implementación es funcionalmente correcta.
2. **Ataque por diccionario.** `brute --format sha256` recupera la palabra
...
3. **`strings` sobre stripped revela toda la superficie CLI** (`--format`,
